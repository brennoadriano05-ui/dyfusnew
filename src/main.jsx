...
import roletaOgrinesImage from '../imagens/roleta/RoletaOgrines.png';
import roletaKamasImage from '../imagens/roleta/roletakamas.png';
...
import vip8Image from '../imagens/loja/vip8.png';
import ogrinesImage from '../imagens/loja/ogrines.png';
...
const products = [
  { name: 'VIP IMPACT 30D', category: 'VIP', type: 'STATUS', price: 'R$ 49,90', detail: '30 dias de benefícios premium', image: vipImage },
  { name: 'VIP IMPACT 8 PERSONAGENS', category: 'VIP', type: 'STATUS', price: 'R$ 19,90', detail: 'Benefícios premium para 8 personagens', image: vip8Image },
  { name: 'VIP IMPACT 4 PERSONAGENS', category: 'VIP', type: 'STATUS', price: 'R$ 12,90', detail: 'Benefícios premium para 4 personagens', image: vip4Image },
  { name: '1.000 OGRINES', category: 'OGRINES', type: 'OGRINES', price: 'R$ 15,00', detail: 'Moeda oficial da loja Impact', image: ogrinesImage },
  { name: '2.000 OGRINES', category: 'OGRINES', type: 'OGRINES', price: 'R$ 30,00', detail: 'Moeda oficial da loja Impact', image: ogrinesImage },
  { name: '5.000 OGRINES', category: 'OGRINES', type: 'OGRINES', price: 'R$ 70,00', detail: 'Moeda oficial da loja Impact', image: ogrinesImage },
  { name: 'Baú do Aventureiro', category: 'ITENS', type: 'ITEM ESPECIAL', price: 'R$ 19,90', detail: 'Itens raros para começar sua jornada', image: ogrinesImage },
  { name: 'Passe de Batalha', category: 'ITENS', type: 'ITEM ESPECIAL', price: 'R$ 29,90', detail: 'Desbloqueie conteúdo exclusivo', image: ogrinesImage }
];
const featuredProducts = [products[1], products[3], products[6]];
...
function App() {
  ...
  return <div className="app">
    ...
    {page !== 'home' ? <Page page={page} jump={jump} notify={notify} /> : <main>
      ...
      <section className="section store-section" id="store"><div className="section-heading"><div><span className="kicker">FORJE SUA JORNADA</span><h2>Loja <em>Impact.</em></h2></div><button className="text-button" onClick={() => jump('store')}>ACESSAR LOJA <ArrowRight size={15} /></button></div><div className="product-grid">{featuredProducts.map(({ name, type, price, detail, image }) => <article className="product-card" key={name}><div className="product-image" style={{ backgroundImage: `url(${image})` }}></div><div className="product-content"><span className="product-type">{type}</span><h3>{name}</h3><p>{detail}</p><div className="product-footer"><strong>{price}</strong><button className="button outline" onClick={() => notify(`${name} adicionado ao carrinho.`)}>ADICIONAR <ShoppingBag size={14} /></button></div></div></article>)}</div></section>
...
function LegacyRoulette({ prizes, freeSpins, ogrines, spinResult, spinning, spinWheel }) { return null; }
  function Roulette({ prizes, freeSpins, ogrines, spinResult, spinning, activePrizeIndex, spinWheel }) { return <div className="roulette-page"><div className="roulette-board"><div className="roulette-title"><span>ROULETA DE RECOMPENSAS</span><h2>CONFIRA OS <em>PRÊMIOS</em></h2><p>1 giro a cada 10 votos ou compre por 250 Ogrines</p></div><div className="prize-board">{prizes.map((prize, index) => <div className={`prize-tile ${index === 2 ? 'featured' : ''} ${spinning && activePrizeIndex === index ? 'prize-active' : ''} ${!spinning && activePrizeIndex === index ? 'prize-winner' : ''}`} key={`${prize.name}-${index}`}><img src={prize.image} alt={`${prize.name} - ${prize.chance}% de chance`} /></div>)}<div className="roulette-result ${spinning ? 'spinning' : ''}"><img src={prizeShowcaseImage} alt="Todos os prêmios da roleta" /><div className="roulette-result-content"><Gift size={25} /><strong>{spinResult}</strong><button className="button primary" disabled={spinning || freeSpins < 1} onClick={() => spinWheel()}>GIRAR GRÁTIS ({freeSpins})</button></div></div></div><div className="wheel-actions"><button className="button outline" disabled={spinning || ogrines < 250} onClick={() => spinWheel(true)}>COMPRAR GIRO · 250 OGRINES <Sparkles size={15} /></button></div><div className="wheel-balance"><span>SEU SALDO <b>{ogrines.toLocaleString('pt-BR')} OGRINES</b></span><span>PRÓXIMO GIRO GRÁTIS <b>10 VOTOS</b></span></div></div><div className="vote-step panel"><span className="kicker">COMO FUNCIONA</span><h2>Vote. Gire.<br /><em>Ganhe recompensas.</em></h2><p>Participe das votações do servidor e acumule giros para concorrer a prêmios especiais.</p>{prizes.map((prize, index) => <div className="prize-line" key={`${prize.name}-${index}`}><span>0{index + 1}</span><strong>{prize.name}</strong><small>{String(prize.chance).replace('.', ',')}%</small></div>)}</div></div>; }
...
function Page({ page, jump, notify }) {
  ...
  const prizes = [
    { name: '500 Ogrines', image: roletaOgrinesImage, chance: 5 },
    { name: 'Ornamento', image: roletaOrnamentoImage, chance: 10 },
    { name: 'VIP por 3 dias', image: roletaVipImage, chance: 7.5 },
    { name: '250.000 Kamas', image: roletaKamasImage, chance: 25 },
    { name: 'Ornamento', image: roletaOrnamentoImage, chance: 10 },
    { name: 'Título exclusivo', image: roletaTituloImage, chance: 5 },
    { name: '1.000 Ogrines', image: roletaOgrinesImage, chance: 2.5 },
    { name: '100.000 Kamas', image: roletaKamasImage, chance: 35 }
  ];
    const spinWheel = (paid = false) => {
    if (spinning || (paid ? ogrines < 250 : freeSpins < 1)) return;
    setSpinning(true);
      let roll = Math.random() * 100;
      let winner = prizes.length - 1;
      for (let prizeIndex = 0; prizeIndex < prizes.length; prizeIndex += 1) {
        if (roll < prizes[prizeIndex].chance) {
          winner = prizeIndex;
          break;
        }
        roll -= prizes[prizeIndex].chance;
      }
      let current = 0;
    ...
    if (paid) setOgrines((value) => value - 250); else setFreeSpins((value) => value - 1);
      spinTimer.current = setInterval(() => {
        ...
        if (current >= prizes.length * 2 + winner) {
          ...
          setSpinResult(prizes[winner].name);
          setPrizeHistory((entries) => [{ prize: prizes[winner].name, time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) }, ...entries]);
          setSpinning(false);
        }
      }, 150);
  };
  ...
  const pageData = {
    ...
    store: { kicker: 'FORJE SUA JORNADA', title: <><img className="page-hero-logo" src={logoPath} alt="Dyfus Impact" /><span>Loja <em>Impact.</em></span></>, intro: 'Itens, VIP e recursos para levar sua aventura ainda mais longe.' },
...
function Modal({ type, close, openSignup, notify, onAuthenticated }) {
  ...
  return <div className={`modal-backdrop ${login ? 'login-backdrop' : ''}`} onClick={close}><div className="modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={close} aria-label="Fechar"><X size={18} /></button><div className="modal-brand"><img src={logoPath} alt="Dyfus Impact" /><span>PORTAL DO AVENTUREIRO</span></div><span className="kicker">{login ? '' : 'JUNTE-SE À LENDA'}</span><h2>{login ? <>Se conectar</> : <>Crie sua <em>conta.</em></>}</h2><p>{login ? <>Você não tem nenhuma conta? <button className="signup-link" onClick={openSignup}>CRIAR CONTA</button></> : 'Seu mundo está esperando por você.'}</p>{!login && <label>LOGIN<input placeholder="Escolha seu nome de aventureiro" /></label>}<label>E-MAIL<input type="email" placeholder="E-mail" value={login ? loginEmail : undefined} onChange={login ? (event) => setLoginEmail(event.target.value) : undefined} /></label>{!login && <><label>PALAVRA SECRETA<select defaultValue=""><option value="" disabled>Escolha uma pergunta de segurança</option><option>Qual era o nome do seu primeiro personagem?</option><option>Qual é o nome do seu lugar favorito?</option><option>Qual era o apelido da sua infância?</option><option>Qual é o nome do seu jogo favorito?</option><option>Qual foi sua primeira guilda?</option></select></label><label>RESPOSTA SECRETA<input placeholder="Digite sua resposta secreta" /></label></>}<label>SENHA<input type="password" placeholder="Senha" value={login ? loginPassword : undefined} onChange={login ? (event) => setLoginPassword(event.target.value) : undefined} /></label>{!login && <label>CONFIRMAR SENHA<input type="password" placeholder="Repita sua senha" /></label>}<button className="button primary full" onClick={login ? submitLogin : () => { onAuthenticated(); close(); notify('Conta demonstrativa criada.') }}>{login ? 'Se Conectar' : 'CRIAR MINHA CONTA'} <ArrowRight size={16} /></button>{login && <button className="modal-helper" onClick={() => setRecover(true)}>Esqueci minha senha?</button>}</div></div>
}
