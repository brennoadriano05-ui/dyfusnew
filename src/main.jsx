...
const products = [
  { name: 'VIP IMPACT 30D', type: 'STATUS', price: 'R$ 49,90', detail: '30 dias de benefícios premium', image: vipImage },
  { name: 'VIP IMPACT 8D', type: 'STATUS', price: 'R$ 19,90', detail: '8 dias de benefícios premium', image: vip8Image },
  { name: 'VIP IMPACT 4D', type: 'STATUS', price: 'R$ 12,90', detail: '4 dias de benefícios premium', image: vip4Image },
  { name: '2.500 OGRINES', type: 'OGRINES', price: 'R$ 24,90', detail: 'Moeda oficial da loja Impact', image: ogrinesImage }
];
...
function App() {
  ...
  return <div className="app">
    ...
    {page !== 'home' ? <Page page={page} jump={jump} notify={notify} /> : <main>
      ...
      <section className="section store-section" id="store"><div className="section-heading"><div><span className="kicker">FORJE SUA JORNADA</span><h2>Loja <em>Impact.</em></h2></div><button className="text-button" onClick={() => notify('Carrinho vazio.')}>VER CARRINHO <ShoppingBag size={15} /></button></div><div className="product-grid">{products.map(({ name, type, price, detail, image }) => <article className="product-card" key={name}><div className="product-icon" style={{ backgroundImage: `url(${image})` }}></div><span>{type}</span><h3>{name}</h3><p>{detail}</p><strong>{price}</strong><button className="button outline" onClick={() => notify(`${name} adicionado ao carrinho.`)}>ADICIONAR <ShoppingBag size={14} /></button></article>)}</div></section>
