...
const products = [
  ...
  { name: 'VIP IMPACT 8D', category: 'VIP', type: 'STATUS', price: 'R$ 19,90', detail: '8 dias de benefícios premium', image: vip8Image },
  { name: 'VIP IMPACT 4D', category: 'VIP', type: 'STATUS', price: 'R$ 12,90', detail: '4 dias de benefícios premium', image: vip4Image },
  { name: '2.500 OGRINES', category: 'OGRINES', type: 'OGRINES', price: 'R$ 24,90', detail: 'Moeda oficial da loja Impact', image: ogrinesImage },
...
function App() {
  ...
  return <div className="app">
    ...
    {page !== 'home' ? <Page page={page} jump={jump} notify={notify} /> : <main>
      ...
      <section className="section store-section" id="store"><div className="section-heading"><div><span className="kicker">FORJE SUA JORNADA</span><h2>Loja <em>Impact.</em></h2></div><button className="text-button" onClick={() => notify('Carrinho vazio.')}>VER CARRINHO <ShoppingBag size={15} /></button></div><div className="product-grid">{products.filter(p => p.category === 'VIP').slice(0, 3).map(({ name, type, price, detail, image }) => <article className="product-card" key={name}><div className="product-image" style={{ backgroundImage: `url(${image})` }}></div><div className="product-content"><span className="product-type">{type}</span><h3>{name}</h3><p>{detail}</p><div className="product-footer"><strong>{price}</strong><button className="button outline" onClick={() => notify(`${name} adicionado ao carrinho.`)}>ADICIONAR <ShoppingBag size={14} /></button></div></div></article>)}</div></section>
